...
export class SourceRepository {
  ...
  async findByCode(code: string) {
    return prisma.source.findFirst({ where: { code } });
  }

  async findByCodeAndProject(code: string, projectId: string) {
    return prisma.source.findFirst({ where: { code, projectId } });
  }
  ...
  async searchByName(projectId: string, name: string) {
    return prisma.source.findMany({
      where: { projectId, name: { contains: name, mode: 'insensitive' } },
      orderBy: { creationDate: 'desc' },
    });
  }
  ...
  async getNextCode(projectId: string): Promise<string> {
    ...
    return `SRC-${counter.toString().padStart(3, '0')}`;
  }
  ...
  private async getNextCounter(projectId: string): Promise<number> {
    ...
    try {
        ...
        where: {
            entity_contextId: { entity: 'SOURCE', contextId: projectId },
        },
        update: { counter: { increment: 1 } },
        create: { entity: 'SOURCE', contextId: projectId, counter: 1 },
        });
